...
const tagWordLength = 41;
const maxLineBreakLength = 22;
...
async function readAsciiFromCLPAddress(tagAddress) {
    const data = await driver.promises.getData(tagAddress, tagWordLength);
...
async function updateInfoPopup() {
    const tag = await readAsciiFromCLPAddress(clp.TagEquipamentoCLP);
    const nome = await readAsciiFromCLPAddress(clp.NomeEquipamentoCLP);
    ...
    const [
        velocidade, freq, corrente, ligado, falha, manAuto,
        intertrav, falhaEmerg, fimCurso, inversor
    ] = valores.map(d => d.values);

    // Setar todos os dados na IHM
    const writes = [
        [ihm.TagEquipamentoIHM, tag],
        [ihm.NomeEquipamentoIHM, nome],
        [ihm.VelocidadeRpmIHM, velocidade],
        [ihm.FrequenciaIHM, freq],
        [ihm.CorrenteIHM, corrente],
        [ihm.StatusLigadoIHM, ligado],
        [ihm.StatusFalhaIHM, falha],
        [ihm.StatusManualAutomaticoIHM, manAuto],
        [ihm.StatusIntertravamentoIHM, intertrav],
        [ihm.StatusFalhaEmergIHM, falhaEmerg],
        [ihm.StatusFimDeCursoIHM, fimCurso],
        [ihm.StatusInStsIHM, inversor]
    ];

    writes.forEach(([addr, value]) => {
        if (typeof value === 'string') {
            driver.setStringData(addr, tagWordLength, value);
        } else {
    ...
    driver.getData(ihm.KeypadSetPointHabilitadoIHM, 1, (_err, data) => {
        if (data.values[0] === 0) {
            driver.setData(ihm.SetPointIHM, freq);
        }
    });
}
...
async function checkCircuit() {
    ...
    const data = await driver.promises.getData(ihm.MotorAtualClickIHM, tagWordLength);
...
mouseArea.on('click', ({ x, y }) => {
    if (x >= MouseAreaStartX && x <= MouseAreaStartX + MouseAreaWidth &&
        ...
        readAsciiFromCLPAddress(clp.TagEquipamentoCLP).then(tag => {
            driver.setStringData(ihm.MotorAtualClickIHM, tagWordLength, tag);
        });
